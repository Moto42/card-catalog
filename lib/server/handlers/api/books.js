...
booksDB.get = function(requestData,callback){
  ...
  database.query('SELECT * FROM books',function(err,books){
    if(!err){

      database.query( 'SELECT * FROM genre', function(err,genres){
        books.forEach(book=>book.genre = []);
        genres.forEach(genre=>{
          const index = books.findIndex(b=>b.id===genre.book)
          if(index > -1){
            books[index].genre.push(genre.genre);
          }
        })

        database.query('SELECT * FROM subjects', function(err,subjects){
          books.forEach(book=>book.subject = []);
          subjects.forEach(subject=>{
            const index = books.findIndex(b=>b.id===subject.book)
            if(index > -1){
              books[index].subject.push(subject.subject);
            }
          })
